...
@injectable()
export class SpotifyAuthController implements IController {
    baseRoute = "/spotify";
    ...
    private async callback(
        ...
        if (state === null || state !== req.state[Cookies.SpotifyAuthState]) {
            ...
            try {
                ...
                const authResp = new URLSearchParams(response.data);
                return h.redirect(
                    `${
                        this.apiConfig.frontendUrl
                    }/#/spotify/success?${authResp.toString()}`
                );
            } catch (error: any) {
